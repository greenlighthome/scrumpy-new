// Thanks to @hoangdov for https://github.com/particle4dev/upload-avatar-meteor
...
Template.editAvatar.events({
        ...
        if(inputImgUpload.files && inputImgUpload.files[0]){
            FileReaderObj.prev(inputImgUpload.files[0], function(err, file){
                if (err){
                    ...
                    $(function () {
                        displayImage.imgAreaSelect({aspectRatio: '1:1', handles: true, fadeSpeed: 200, parent: $('#edit-avatar'), onSelectChange: preview});
                    });
                }
            });
        }
    ...
    'click #save-avatar-button': function(e, t){
        e.preventDefault();
        if (Session.equals('removeAvatar', true)) {
            Meteor.call('setDefaultAvatar', Meteor.userId(), function(err, res){
                if (err){
                    throwAlert('error', 'Sorry!', err.message);
                }
            });
...
function displaySuccessAlertForAvatarChanged() { // TODO: Bug
